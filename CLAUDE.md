...
### Admin middleware
```ts
// Новый способ (приоритет):
Authorization: Bearer <jwt>   // JWT подписан ADMIN_JWT_SECRET, 8 часов

// Старый fallback (backward compat):
X-Admin-Code: <plaintext>     // deprecated, убрать после миграции клиентов
```
...
| `ADMIN_ACCESS_CODE` | Пароль для `/admin/auth` |
...
**Критично:** `ADMIN_JWT_SECRET` ещё не добавлен в Supabase Secrets — без него JWT не выдаётся, работает только legacy `X-Admin-Code`.
