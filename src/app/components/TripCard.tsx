/**
 * TripCard — единая карточка поездки/объявления.
 * Используется везде: DriverTripsPage, SenderTripsPage, SearchResults.
 * mode='driver'  → полный контроль (статус, оферты, кнопки управления)
 * mode='sender'  → вид бронирования (инфо о водителе, чат, трекинг)
 * mode='search'  → объявление (инфо о водителе, кнопка «Подробнее»)
 */
...
export function TripCard({
  ...
  // Fix #5: убран бессмысленный if/else — обе ветки были идентичны
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 flex-wrap">
            {/* Show status badge on driver and sender pages */}
        ...
        {isCargoTrip && cargoWeight > 0 && (
          <div className="flex items-center gap-1.5 flex-wrap">
            <div className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-amber-500/10 border border-amber-500/20">
              <Package className="w-3 h-3 text-amber-400" />
              <span className="text-[11px] font-bold text-white">{cargoWeight} кг груза</span>
            </div>
            {budget > 0 && (
              <div className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
        ...
        {(trip.vehicle || creatorPhone) && (
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* Phone shown in specific modes or after booking */}
        ...
        {/* ── DRIVER MODE: Pending offers banner */}
        {/* Fix #9: pending offers banner → /messages (чаты отправителей), а не в TripDetail где нет списка оферт */}
        {mode === 'driver' && (trip.pendingOffersCount ?? 0) > 0 && !isInactive && (
          <button
            onClick={e => { e.stopPropagation(); navigate('/messages'); }}
            className="w-full flex items-center gap-2 px-3 py-2.5 rounded-xl bg-amber-500/10 border border-amber-500/30 active:scale-[0.98] transition-all"
          >
            <Package className="w-3.5 h-3.5 text-amber-400 shrink-0" />
            <span className="text-[12px] font-bold text-amber-400 flex-1 text-left">
              {trip.pendingOffersCount} {trip.pendingOffersCount === 1 ? 'новая оферта' : (trip.pendingOffersCount ?? 0) < 5 ? '��овые оферты' : 'новых оферт'} — ответить
            </span>
        ...
        {/* Fix #7: sender completed — кнопка «Написать» всегда доступна (поддержка, претензии) */}
        ...
        {/* ── DRIVER mode: Delete button for inactive trips (completed / cancelled) */}
        {mode === 'driver' && isInactive && onDelete && (
          <div onClick={e => e.stopPropagation()}>
            <button
              onClick={onDelete}
              title="Удалить поездку"
              className="w-full h-9 flex items-center justify-center gap-2 rounded-xl bg-rose-500/08 border border-rose-500/15 text-rose-400/70 hover:bg-rose-500/15 hover:text-rose-400 transition-all active:scale-95 text-[11px] font-semibold"
            >
        ...
        {mode === 'driver' && !isInactive && (
          <div className="flex items-center gap-2 pt-0.5" onClick={e => e.stopPropagation()}>
            ...
            {/* Icon buttons — набор зависит от статуса */}
            <div className="flex items-center gap-1.5">
              {/* Freeze (planned → frozen) */}
              {trip.status === 'planned' && (
                <button onClick={onFreeze} title="Заморозить"
              ...
              {/* Pause (inProgress → frozen) */}
              {trip.status === 'inProgress' && (
                <button onClick={onFreeze} title="Поставить на паузу"
              ...
              {/* Complete — только для inProgress */}
              {trip.status === 'inProgress' && (
                <button onClick={onComplete} title="Завершить поездку"
              ...
              {/* Cancel (planned/frozen → cancelled) — только для неактивного рейса */}
              {(trip.status === 'planned' || trip.status === 'frozen') && (
                <button onClick={onCancel} title="Отменить поездку"
              ...
              {/* Messages */}
              <button onClick={onMessages} title="Сообщения"
              ...
              {/* Trash2 убран — теперь только для неактивных рейсов (см. выше) */}
            </div>
          </div>
        )}
          ...
          <div className="flex items-center gap-2 pt-0.5" onClick={e => e.stopPropagation()}>
            {trip.status === 'inProgress' && (
              <>
                ...
                {/* Подсказка: отмена недоступна в пути */}
                <div title="Отмена недоступна — рейс в пути"
            ...
            {/* Fix #3: frozen рейс у sender — информируем, трекинг недоступен */}
            {trip.status === 'frozen' && (
              <div className="flex items-center gap-2 w-full">
                ...
                {/* Отменить бронь доступно и при frozen */}
                <button onClick={onCancelBooking} title="Отменить бронирование"
                  className="w-10 h-10 rounded-xl flex items-center justify-center bg-rose-500/10 border border-rose-500/20 text-rose-400 hover:bg-rose-500/20 transition-all active:scale-90">
                  <XCircle className="w-4 h-4" />
                </button>
              </div>
            ...
            <button onClick={onChat} title="Написать водителю"
              className="relative w-10 h-10 rounded-xl flex items-center justify-center bg-[#5ba3f5]/10 border border-[#5ba3f5]/20 text-[#5ba3f5] hover:bg-[#5ba3f5]/20 transition-all active:scale-90">
              <MessageSquare className="w-4 h-4" />
              {unreadMessages > 0 && (
                <span className="absolute -top-1 -right-1 min-w-[16px] h-4 px-0.5 rounded-full bg-rose-500 text-white text-[9px] font-black flex items-center justify-center">
        ...
        {/* ── SEARCH mode CTA */}
        {/* Fix #6: e.stopPropagation() предотвращает двойную навигацию (карточка + кнопка) */}
        {mode === 'search' && (
          <button
            onClick={e => { e.stopPropagation(); navigate(`/trip/${tid}`); }}
            className="w-full h-10 flex items-center justify-center gap-2 rounded-xl bg-[#1978e5] hover:bg-[#1565cc] text-white text-[13px] font-bold transition-all active:scale-95 shadow-lg shadow-[#1978e5]/20"
          >
