...
import { getBlacklist, removeFromBlacklist } from '../../api/dataApi';
...
export function BlacklistManagement() {
  ...
  const handleRemove = async (entry: any) => {
    if (!confirm(`Снять блокировку с номера ${entry.phone}? Этот номер снова сможет регистрироваться.`)) return;
    setActionLoading(entry.phone);
    try {
      await removeFromBlacklist(entry.phone);
  ...
  const filtered = entries.filter(e => {
    ...
    return !q
      ...
      || (e.originalEmail || '').toLowerCase().includes(q);
  });
  ...
  return (
    <div className="space-y-5">
      <AdminPageHeader
        title="Чёрный список"
        subtitle="Номера телефонов, заблокированные после удаления аккаунта (CARGO и AVIA)"
        icon={ShieldOff}
      ...
      <div className="bg-white rounded-2xl overflow-hidden" style={{ border: '1px solid #f0f4f8' }}>
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-gray-500 font-medium">Чёрный список пуст</p>
            <p className="text-gray-400 text-sm mt-1">Удалённые аккаунты появятся здесь</p>
          </div>
        ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap mt-1">
                      <span
                        ...
                        style={entry.source === 'avia' ? { background: '#e0f2fe', color: '#0369a1' } : { background: '#eff6ff', color: '#1d4ed8' }}
                      >
                        {entry.source === 'avia' ? '✈ AVIA' : '📦 CARGO'}
                      </span>
                      {entry.originalRole && (
                    ...
                    {(entry.originalName || entry.originalEmail) && (
                      <p className="text-xs text-gray-400 mt-1 truncate">
                        {[entry.originalName, entry.originalEmail].filter(Boolean).join(' • ')}
                      </p>
                    )}
